...
* **มาตรฐานการแสดงผลแผนภาพและตาราง (Mermaid & Markdown Tables Only)**:
  - แผนภาพ ผังเครือข่าย และโครงสร้างระบบทั้งหมด ทั้งในการตอบบน Chat และการบันทึกไฟล์รายงาน ต้องใช้บล็อก **Mermaid** เสมอ โดยห้ามใช้ ASCII Text Art หรือ Unicode Box Drawing
  - ตารางเปรียบเทียบข้อมูล พารามิเตอร์ และเมทริกซ์ทั้งหมด ต้องใช้มาตรฐาน **Markdown Table** (`| ... |`) เสมอ ห้ามใช้การจัดช่องว่างตารางแบบ ASCII
* **ข้อกำหนดคำแนะนำเครื่องมือเสริม (Chat-Only Delivery & No PDF/HTML/DOCX/ODT Export)**:
  - ระบบตัดการแปลงไฟล์เป็น PDF, HTML, DOCX, ODT ออกโดยเด็ดขาด การบันทึกรายงานทั้งหมดจะใช้เฉพาะ **Markdown (`.md`)** และ **JSON (`.json`)** เป็น Single Source of Truth
  - หากผู้ใช้ต้องการคำแนะนำในการพิมพ์หรือดูไฟล์ Markdown เป็น PDF (เช่น การใช้ Web Browser พิมพ์ผ่าน `Ctrl + P` หรือเปิดด้วยโปรแกรม Markdown Editor ภายนอก) คำแนะนำจะแสดงผลเฉพาะในหน้าต่างสนทนา (Chat) เท่านั้น โดยไม่มีการบันทึกลงในไฟล์หรือไดเรกทอรีรายงาน
* **การจัดเก็บรายงานในไดเรกทอรี `reports/`**:
  - เมื่อสั่งให้บันทึกรายงาน ผลลัพธ์จะถูกบันทึกเป็นไฟล์ Markdown (.md) หรือ JSON (.json) ในโฟลเดอร์ [`reports/`](reports/) โดยใช้การเข้ารหัส UTF-8 ตามข้อกำหนดสากลเสมอ ปราศจากการแปลงเป็นไฟล์ฟอร์แมตปิดอื่นๆ
